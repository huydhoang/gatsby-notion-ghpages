...
def process_block(_slug, block, text_prefix=''):
    ...
    for content in block.children:
        ...
        if content.type == 'header':
            ...
            _file_path = os.path.join(dest_path, _slug)
            # print(_file_path)
            image_name = download_file(content.source, _file_path)
...
def to_markdown(page_id, ignore):
    ...
    raw_page = page.get()
    # print(raw_page)
    # print(raw_page.get('last_edited_time'))
    # sys.exit()
    if 'format' in raw_page and 'page_cover' in raw_page['format']:
        ...
        os.mkdir(_file_path)
        # sys.exit()
        cover_image_name = download_file(page_cover_url, _file_path)
        metas.append(f"thumbnail: '{cover_image_name}'")

    text, child_metas = process_block(_slug, page)
...
if __name__ == "__main__":
    ...
    # Fetch markdown data to memory
    to_markdown(root_page_id, ignore=ignore_root)

    # Write markdown data to files
    for slug, markdown in markdown_pages.items():
        file_path = os.path.join(dest_path, slug)
        # file_name = slug + '.md'
    ...
    # Success message
    print(f'Done: imported {str(len(markdown_pages))} pages.')
